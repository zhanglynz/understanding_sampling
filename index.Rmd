...
Sampling is challenging, because not only must we deal with mathematics but also we must cope with many practical issues, such as defining population, reducing cost (as much as we can), and non-response etc. etc.. Part I of this book is mathematics oriented, and Part II focuses on practical things in sampling.

# (PART) Mathematical part {-}
...
- There are ${10 \choose 3}=120$ possible samples. In practice, we can only have one sample, and there will always be **uncertainty** if we use one sample to infer the whole population.
...
- The distribution of all possible sample variances (NB: the divisor is $n-1$) is shown below.  
```{r, message=FALSE, warning=FALSE, echo=FALSE}
hist(s2, freq = FALSE)
```
