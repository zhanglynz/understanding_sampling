...
- There are ${10 \choose 3}=120$ possible samples---here we are talking about *simple random samples* (SRS). In practice, we can only have one sample, and there will always be **uncertainty** if we use one sample to infer the whole population.
...
(\#eq:pisum)
\end{equation}
Actually, \@ref(eq:pisum) is always hold regardless of sampling schemes; below is a quick proof.

**Proof:** 
\begin{align}
\sum_{i=1}^N \pi_i&= \frac{Nn(n-1)!\binom{N-1}{n-1}}{n!\binom{N}{n}}\\
&=n.
\end{align}
